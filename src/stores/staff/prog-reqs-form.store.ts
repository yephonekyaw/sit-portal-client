...
interface ProgramRequirementFormData {
  id?: string;
  name: string;
...
interface ProgramRequirementFormState {
  ...
  saveRequirement: (requirementData: ProgramRequirementFormSchemaType & { program: { program_code: string }; certificate_type: { code: string; name: string }; id?: string }) => Promise<void>;
}

export const useProgramRequirementFormStore = create<ProgramRequirementFormState>((set, get) => ({
  ...
  loadRequirement: async (requirementId: string) => {
    ...
    try {
      ...
      const requirement = programRequirements.find(r => r.id === requirementId);
      if (!requirement) {
      ...
      set({ 
        error: error instanceof Error ? error.message : "Failed to load program requirement",
        isLoading: false 
      });
    }
  },

  saveRequirement: async (requirementData: ProgramRequirementFormSchemaType & { program: { program_code: string }; certificate_type: { code: string; name: string }; id?: string }) => {
    set({ isLoading: true, error: null });
    ...
    try {
      ...
      if (requirementData.id) {
        ...
        const updatedRequirement: ProgramRequirement = {
          ...
          special_instruction: requirementData.special_instruction || null,
      ...
      set({ 
        error: error instanceof Error ? error.message : "Failed to save program requirement",
        isLoading: false 
      });
      throw error;
    }
